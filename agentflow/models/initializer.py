...
class Initializer:
    ...
    def load_tools_and_get_metadata(self, parallel: bool = True, max_workers: int = 4) -> Dict[str, Any]:
        ...
        if self.load_all:
            ...
            for tool_dir in self.available_tools:
                if tool_dir in dir_to_paths:
                    ...
                    print(f"[warn]  Warning: Tool directory '{tool_dir}' not found in tools directory")
        ...
        if parallel and len(tool_dirs_to_process) > 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                print(f"[wait] Waiting for all {len(futures)} modules to load...")
                ...
                for future, import_path in futures:
                    try:
                        ...
                        for instance_key, instance in result['instance_list']:
                            ...
                            print(f"[ok] Loaded: {instance_key} with engine: {getattr(instance, 'model_string', 'default')}")
                    except Exception as e:
    ...
    def run_demo_commands(self) -> List[str]:
        ...
        for tool_name, tool_data in sorted(self.toolbox_metadata.items()):
            ...
            try:
                ...
                if tool_name in self.tool_instances_cache:
                    ...
                    print(f"[ok] Using cached instance with engine: {getattr(tool_instance, 'model_string', 'default')}")
                else:
                    ...
                    print(f"[warn]  Created new instance (not in cache)")
        ...
        print("\n[done] Finished running demo commands for each tool.")
    ...
    def _set_up_tools(self) -> None:
        ...
        print("[done] Finished setting up tools.")
        print(f"[done] Total number of final available tools: {len(self.available_tools)}")
        print(f"[done] Final available tools: {self.available_tools}")
...
if __name__ == "__main__":
    ...
    print(f"\n[ok] Serial loading completed in {serial_time:.2f}s")
    ...
    print(f"\n[ok] Parallel loading completed in {parallel_time:.2f}s")
