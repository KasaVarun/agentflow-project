...
### Step 3: Model Scaling Study (Qwen3.5 family, Bamboogle)
...
Strong performance at 2B–4B; larger models (9B, 27B) regress on Bamboogle, likely due to over-hedging or instruction-following differences in the Qwen3.5 series.

### Step 4: Text-to-SQL (Spider Dev, Qwen3.5 family)

Evaluated on Spider dev using execution accuracy.
...
All Qwen3.5 sizes exceed the Qwen2.5-7B baseline (91.01%), with 4B achieving the best result. SQL generation appears much less sensitive to model scale than multi-hop QA.

### Step 5-6: Flow-GRPO Training

- **Step 5**: 50-step Flow-GRPO on Qwen3.5-0.8B-Instruct (QA tasks: NQ + HotpotQA)
  - LoRA: r=16, alpha=64, target modules: q/k/v/o_proj
  - Group size G=8, PPO clip ε=0.2, KL coef=0.01
  - Checkpoint: Modal Volume `agentflow-checkpoints/flow_grpo_0.8b`

- **Step 6**: 50-step Flow-GRPO on Qwen3.5-0.8B-Instruct (Spider SQL)
  - Same LoRA config, benchmark="sql"
  - Checkpoint: Modal Volume `agentflow-checkpoints/flow_grpo_sql_0.8b`
...
PYTHONUTF8=1 modal run modal_deploy/train_flow_grpo.py \
    --experiment-name flow_grpo_0.8b --max-steps 50
